...
이 문서는 신규 학습과 보수적인 복습에 사용하는 기본 실행안이다. 42 과정을 이미 수료했고 C·C++·WEB 프로젝트 세 개를 동시에 유지할 수 있다면 [3트랙 병렬 PATH](11-PARALLEL-PATH.md)를 대체 실행 모드로 사용할 수 있다. 대체 모드는 각 트랙 안에 G 구간과 P를 선형으로 교차 배치하며, P만 원자적으로 완료하고 필수 G 범위는 시점별로 나누어 모두 이수한다.
