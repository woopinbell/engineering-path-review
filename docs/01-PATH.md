...
이 문서는 신규 학습과 보수적인 복습에 사용하는 기본 실행안이다. 42 과정과 기존 WEB·SPORTSBOOK 구간을 이미 수료했고 C·C++·WEB·SPORTSBOOK 네 트랙을 동시에 유지할 수 있다면 [4트랙 병렬 PATH](11-PARALLEL-PATH.md)를 대체 실행 모드로 사용할 수 있다. 대체 모드는 각 트랙 안에 G 구간과 P를 선형으로 교차 배치하며, P만 원자적으로 완료하고 필수 G 범위는 시점별로 나누어 모두 이수한다. 트랙 간에는 실제 선행 관계만 하드 동기화한다.
