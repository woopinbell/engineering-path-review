...
`L`은 `P` 프로젝트나 `G` 가이드처럼 처음부터 끝까지 이어지는 세 번째 학습 경로가 아니다.
...
감사 도중 구현·대규모 문서 재작성·새 학습 범위가 필요해지면 현재 `L`을 확장하지 않는다. 해당 문제를 `P` 또는 `G`의 수정 작업으로 이동하고 `L`에는 발견 근거와 후속 owner만 남긴다. 이 규칙이 없으면 `L`이 보이지 않는 세 번째 무거운 레인이 된다.
...
| L04 | 계열 감사 | P06 + G02 | C 계열 P01~P06의 소유권·I/O·process/thread 연결 | 모순·중복·누락 P0/P1 목록 | 필수 1회 |
| L05 | 계열 감사 | P11 + G03 | C++·시스템 P07~P11의 객체·container·oracle·event loop 연결 | 개념 owner와 재설명 조건 확정 | 필수 1회 |
...
| L12 | 계열 감사 | P19 + G12~G14 | sportsbook 기반 P16~P19의 schema·DB·Redis·Kafka 보장 연결 | 기반 서비스의 정본·전달 보장 확정 | 필수 1회 |
| L13 | 계열 감사 | P24 | sportsbook P16~P24 전체 data/control/runtime/evidence plane | 전체 P0/P1=0, 교차 링크·E2E 근거 확정 | 필수 1회 |
| L14 | 지원 준비 | P14 공개·검증 완료 | 첫 지원 전 이력서·대표 repo·5분 설명·모의 1회 | 1차 지원 가능한 최소 패키지 | 필수 1회 |
| L15 | 지원 갱신 | P15/P17/P20/P22 도달 | 새 직군의 대표 repo·공고 등급·이력서 문구만 갱신 | 다음 지원 파동 준비 완료 | 마일스톤별 |
