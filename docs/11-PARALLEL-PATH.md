...
그래프에서는 네 checkpoint만 같은 황색 계열의 동기화 노드로 표시한다. 하드 동기화 전체를 하나의 빨강으로 덮지 않고, 관계 표의 제공 측 표시는 선행 범위를 제공하는 출발 트랙 색상을 유지한다. 따라서 같은 노드 색은 “여기서 완료 조건을 확인한다”는 뜻이고, 표의 출발 색은 “어느 트랙이 선행 범위를 제공하는가”를 뜻한다.
