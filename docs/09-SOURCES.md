...
| 복습용 병렬 PATH 재정의 논의 | C·C++·WEB·SPORTSBOOK 트랙 분리, 트랙 내부 G·P 선형 배치, P만 원자적 완료, 필수 G 비생략, 직접 선행 관계의 하드 동기화 |
...
C·C++·WEB·SPORTSBOOK 4트랙 안에 가이드 구간과 프로젝트를 교차 배치한 복습용 실행 설계
P09를 P03 뒤로 옮기고 Algorithms 01~10만 하드 게이트로 남긴 결정
P22·P24에 필요한 WEB 범위만 SPORTSBOOK 트랙과 하드 동기화한 결정
