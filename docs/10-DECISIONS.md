...
- **결정:** 기본 PATH와 별도로 C·C++·WEB을 병렬 진행하고, 각 트랙 내부에는 필요한 가이드 구간과 프로젝트를 하나의 선형 순서로 교차 배치한다.
- **결과:** 프로젝트만 원자적으로 완료한다. 가이드 저장소 전체의 원자성은 해제하지만 필수 장·실습 범위는 제거하지 않고 PATH의 지정 위치에서 모두 이수한다.
- **의존성:** 강한 트랙 간 동기화는 `guide-algorithms` 01~10 → P09, `guide-computer-networks` 01~11 → network 12의 두 지점으로 제한한다.
- **경계:** P09는 C 트랙 마지막 프로젝트로 배치하지만 P 번호와 정본은 바꾸지 않는다. P16~P24는 기본 PATH를 유지한다.
